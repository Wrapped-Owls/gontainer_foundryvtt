...
func TestMutateFile_corruptJSONPreservesFile(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "WriteProfiles/corrupt json is rejected without touching the file",
			write:     writeProfiles,
			writeFile: true,
			content:   corrupt,
			wantErr:   true,
		...
		{
			name:      "WriteProfiles/a missing file still writes fine",
			write:     writeProfiles,
			writeFile: false,
		},
		{
			name:      "WriteProfiles/an empty file still writes fine",
			write:     writeProfiles,
			writeFile: true,
			content:   "",
		},
		{
			name:      "WriteActive/corrupt json is rejected without touching the file",
			write:     writeActive,
			writeFile: true,
			content:   corrupt,
			wantErr:   true,
		...
		{
			name:      "WriteActive/a missing file still writes fine",
			write:     writeActive,
			writeFile: false,
		},
		{
			name:      "WriteActive/an empty file still writes fine",
			write:     writeActive,
			writeFile: true,
			content:   "",
		},
