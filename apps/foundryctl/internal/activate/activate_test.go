...
func TestPrepare_NoNetwork(t *testing.T) {
	...
	profileState, err := PrepareProfile(
		...
		state,
		profile.Profile{Version: foundryVersion13},
	)
	if err != nil {
	...
	if profileState.JSRuntime.Kind != jsruntime.Node ||
		profileState.JSRuntime.Path != wantRuntimePath13 {
		t.Fatalf(
			"profile js runtime = %+v, want node at %q",
			profileState.JSRuntime,
			wantRuntimePath13,
		)
	}
}
