...
func EnsureInstall(
	...
	if cfg.Install.ReleaseURL != "" {
		root, installed, urlErr := acquireLatestFromURL(
			ctx,
		...
		if urlErr != nil {
			return Install{}, urlErr
		}
		return Install{Root: root, Info: installed}, nil
	}
...
func resolveVersioned(
	...
	installed, err := lifecycle.DetectInstalled(targetRoot)
	if err != nil {
		return Install{}, fmt.Errorf("detect install after acquisition: %w", err)
	}
	if !versionMatches(installed.Version, cfg.Install.Version) {
		return Install{}, fmt.Errorf(
			"installed Foundry version %q does not satisfy desired version %q",
			installed.Version, cfg.Install.Version,
		)
	}
	return Install{Root: targetRoot, Info: installed}, nil
}
