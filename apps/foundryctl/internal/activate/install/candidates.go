...
func scanInstallCandidates(installRoot string) ([]installCandidate, error) {
	...
	for _, entry := range entries {
		...
		installed, derr := lifecycle.DetectInstalled(child)
		if derr != nil {
			return nil, fmt.Errorf("detect install %s: %w", child, derr)
		}
		if installed.Present {
			out = append(out, newCandidate(child, installed))
		}
	}
...
func newCandidate(path string, installed lifecycle.InstalledInfo) installCandidate {
	c := installCandidate{Path: path, Info: installed, Version: installed.Version}
	if v, err := semver.NewVersion(installed.Version); err == nil {
		c.Parsed = v
		c.Version = v.String()
	}
	return c
