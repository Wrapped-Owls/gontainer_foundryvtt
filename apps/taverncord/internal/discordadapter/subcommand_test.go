...
func subOption(name, value string) *discordgo.ApplicationCommandInteractionDataOption {
...
func focusedSubOption(name, value string) *discordgo.ApplicationCommandInteractionDataOption {
	opt := subOption(name, value)
	opt.Focused = true
	return opt
}
...
func TestParseInvocationFindsTheSubcommand(t *testing.T) {
	...
	inv, isSub := parseInvocation(subData(subSwitch, subOption(optionName, profAlice)))
	if !isSub {
...
func TestParseInvocationRejectsWhatIsNotASubcommand(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			data: discordgo.ApplicationCommandInteractionData{
				Options: []*discordgo.ApplicationCommandInteractionDataOption{
					subOption(optionName, profAlice),
				},
			},
		},
	}
...
func TestFocusedOption(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "no options means nothing is focused"},
		{
			...
			opts: []*discordgo.ApplicationCommandInteractionDataOption{
				subOption(optionLabel, "x"),
				focusedSubOption(optionName, "ali"),
			},
			wantName:  optionName,
			wantTyped: "ali",
		},
