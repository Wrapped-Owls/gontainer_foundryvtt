	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				status, err := makeCommands(&client).awaitReady(
					context.Background(),
				...
				if status.World != testCase.wantWorld {
					t.Fatalf("world = %q, want %q", status.World, testCase.wantWorld)
				}
			})
		})
	}
