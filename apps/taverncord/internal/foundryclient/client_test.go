...
func TestFastCallsUseShortContextTimeout(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"ListProfiles",
			func(c *Client) error { _, err := c.ListProfiles(context.Background()); return err },
		},
		{
			"Versions",
			func(c *Client) error { _, err := c.Versions(context.Background()); return err },
		},
		{"Status", func(c *Client) error { _, err := c.Status(context.Background()); return err }},
		{
			"GetProfile",
			func(c *Client) error { _, err := c.GetProfile(context.Background(), profAlice); return err },
		},
		...
		{
			"Events",
			func(c *Client) error { _, err := c.Events(context.Background(), 0); return err },
		},
		{
			"Switch",
			func(c *Client) error { return c.Switch(context.Background(), profAlice, command.InterruptWhenIdle) },
		},
		{
			"Restart",
			func(c *Client) error { return c.Restart(context.Background(), command.InterruptWhenIdle) },
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if remaining <= 0 || remaining > dashboardRequestTimeout+slack {
				t.Fatalf(
					"deadline %v from call start, want within (0, %v]",
					remaining,
