...
func TestCopy_FilesAndDirs(t *testing.T) {
	...
	packageJSONStat, err := os.Stat(filepath.Join(dst, "resources", "app", "package.json"))
	if err != nil {
		t.Fatal(err)
	}
	if packageJSONStat.Mode().Perm() != 0o600 {
		t.Errorf("package.json mode = %v, want 0o600", packageJSONStat.Mode().Perm())
	}
}
