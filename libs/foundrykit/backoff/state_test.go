...
func TestWriteStateAtomicIsAtomic(t *testing.T) {
	...
	firstWrite := State{ConsecutiveFailures: 1}
	if err := writeStateAtomic(path, firstWrite); err != nil {
		t.Fatal(err)
	}
	secondWrite := State{ConsecutiveFailures: 2}
	if err := writeStateAtomic(path, secondWrite); err != nil {
		t.Fatal(err)
	}
	got, err := readState(path)
	if err != nil {
