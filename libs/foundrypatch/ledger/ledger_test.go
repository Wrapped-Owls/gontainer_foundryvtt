...
func TestHashPatch_StableAndChange(t *testing.T) {
	p := samplePatch()
	firstHash := HashPatch(p)
	secondHash := HashPatch(p)
	if firstHash != secondHash {
		t.Errorf("hash unstable across calls: %q vs %q", firstHash, secondHash)
	}
	editedPatch := p
	editedPatch.Actions[0].Content = "world"
	editedHash := HashPatch(editedPatch)
	if firstHash == editedHash {
